...
2. **Partie II (Données Images) :** Classification spatiale avec un Réseau Convolutif (CNN) inspiré de LeNet-5 sur le dataset *Fashion-MNIST* (avec des fonctions de convolution codées *from scratch*).
...
Conformément aux consignes, les fichiers et la documentation du projet sont organisés selon l'arborescence standardisée suivante :

```text
├── Notebooks/
│   └── Projet_DeepLearning_ERREYYADI_OUIAM(1).ipynb  
├── Documentation/
│   └── Rapport_Projet_DeepLearning_ERREYYADI_OUIAM.pdf                                                  
└── README.md                                                 
